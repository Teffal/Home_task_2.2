import json
import cchardet as chardet      #cchardet=chardet, but cchardet work faste than chardet
import os
import string
...
def get_frequency_dictionery(our_directory):
    list_files = get_file_list(our_directory, ".json")
    for json_file in list_files:
        strip = string.whitespace + string.punctuation + string.digits + "\"'" + '<br>'
        enc = json_file[1].lower().strip()
        with open(json_file[0], 'r', encoding=enc) as f:
            data = json.load(f)
            words = {}
        ...
        print_frequency_dictionery(json_file[0],words)
...
if __name__ == "__main__":
    ...
    get_frequency_dictionery(our_directory)